...
class SHR_Block(nn.Module):
    def __init__(self, dim, num_heads, mlp_hidden_dim, qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        self.mlp_dual = Mlp(in_features=dim * 3, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop)

    def forward(self, x_1, x_2, x_3):
        x = torch.cat([x_1, x_2, x_3], dim=2)
        x_1 = self.norm1_1(x_1)
        x_2 = self.norm1_2(x_2)
        x_3 = self.norm1_3(x_3)
        x_nor = torch.cat([x_1, x_2, x_3], dim=2)

        x_1 = self.attn_1(self.norm1_1(x_1))
        x_2 = self.attn_2(self.norm1_2(x_2))
        x_3 = self.attn_3(self.norm1_3(x_3))
        x_nor_attn = torch.cat([x_1, x_2, x_3], dim=2)

        x_1 = self.drop_path(self.norm1_1(x_1))
        x_2 = self.drop_path(self.norm1_2(x_2))
        x_3 = self.drop_path(self.norm1_3(x_3))
        x_nor_attn_drop = torch.cat([x_1, x_2, x_3], dim=2)

        x_first = x + x_nor_attn_drop
        x_first = self.norm2(x_first)

        x_two = self.mlp_dual(x_nor)
        x_two = x_two + x_nor_attn
        x_two = self.norm3(x_two)

        x_o = x_first + x_two

        x_o = x_o + self.drop_path(self.mlp(x_o))

        x_1 = x_o[:, :, :x.shape[2] // 3]
        x_2 = x_o[:, :, x.shape[2] // 3: x.shape[2] // 3 * 2]
        x_3 = x_o[:, :, x.shape[2] // 3 * 2: x.shape[2]]
